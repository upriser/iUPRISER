...
- (IBAction)share:(id)sender
{
	if (currentSheet) {
		...
		currentSheet = [[UIActionSheet alloc]
						...
						otherButtonTitles:@"E-mail Link", @"Open Link in Safari", @"Hide on UPRISER", @"Save on UPRISER", @"Save on Instapaper", nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex 
{
	...
	if (buttonIndex == 0)
	{
		...
		if ([MFMailComposeViewController canSendMail])
		{
			...
			if (story.title)
				[controller setSubject:[NSString stringWithFormat:@"\"%@\" from UPRISER", 
										story.title]];
			else
...
- (void)saveOnInstapaper:(id)sender
{
	...
	if (!username || [username isEqual:@""])
	{
		[[[[UIAlertView alloc] initWithTitle:@"Instapaper Error" 
									 message:@"You must provide an Instapaper username to save stories with Instapaper. You can add a username in the UPRISER settings." 
									delegate:nil 
						   cancelButtonTitle:@"OK" 
						   otherButtonTitles:nil] autorelease] show];
		return;
